...
public interface OracleDatabaseAdmin
{
   ...
   * JSON.
   * <p>
   * Passing <code>null</code> for <code>collectionMetadata</code> 
   * and <code>CollectionCreateMode.DDL</code> for <code>mode</code> is equivalent
   * to invoking {@link #createCollection(String)}.
   * <p>
   * Passing <code>CollectionCreateMode.DDL</code> for <code>mode</code> is equivalent
   * to invoking {@link #createCollection(String, OracleDocument)}.
   * <p>
   * If the collection with the specified <code>collectionName</code> exists,
   * it's returned (unless its metadata doesn't match the metadata
   * specified via <code>collectionMetadata</code>,
   ...
   * @param collectionMetadata             implementation-specific collection
   *                                       metadata
   * @param mode                           collection create mode: 
   *                                       {@link CollectionCreateMode#DDL} or
   *                                       {@link CollectionCreateMode#MAP}. If
   *                                       {@link CollectionCreateMode#DDL} is specified,
   *                                       SODA will first attempt to create the
   *                                       table backing the collection. If the table
   *                                       already exists, SODA will attempt to create a
   *                                       collection on top of it.
   *                                       If {@link CollectionCreateMode#MAP} 
   *                                       is specified, SODA will attempt to
   *                                       create a collection on top of an
   *                                       existing table (specified under 
   *                                       <code>tableName</code> field in the metadata).
   *                                       
   * @return OracleCollection              created collection
   * @throws OracleException               if (1) the collection could not
   *                                       be created, or (2) the collection
